                            ...
                            <div class="table-responsive">
                                <table class="table align-items-center table-flush">
                                    <thead class="thead-light">
                                        ...
                                        <th style="min-width: 150px;" scope="col" class="sort" data-sort="Aksi">Action</th>
                                    ...
                                    @endphp
                                        ...
                                        <td>{{ $i++ }}</td>
                                            <th scope="row">
                                                ...
                                                <button type="button" class="btn btn-warning btn-edit" data-toggle="modal" data-target="#editPajak" data-id="{{ $item->id }}"><i class="fas fa-edit"></i></button>
                                                        <form action="{{ route('pajak.destroy',$item->id) }}" method="post" id="delete{{ $item->id }}">
                                                        @csrf
                                                        @method('delete')
                                                    <button class="btn btn-danger" type="button" onclick="deletePajak({{ $item->id }})"><i class="fas fa-trash"></i></button>
                                                    </form>
                                                </div>
                                                </td>
                                            </tr>
...
@section('modal')
     ...
     {{-- Modal Edit Pegawai  --}}
     <div class="modal fade" id="editPajak" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content  mt-5">
                ...
                <div class="modal-body" id="respons-modaleditpajak" >
                   aaa
                </div>
            </div>
        </div>
    ...
    elementSelectPegawai.addEventListener('change', function(){
       ...
       $.ajax({
        ...
        function deletePajak(id){
            Swal.fire({
                title: 'PERINGATAN!',
                text: "Yakin ingin menghapus Pajak?",
                type: 'warning',
            ...
            $('.btn-edit').on('click',function(){
                ...
                $.ajax({
                    ...
                    url:"{{ route('pajak.ViewUpdate','')}}"+"/"+id,
                    method:'GET',
                    success:function(data){
                        // $('#editKendaraan').find('.modal-body').html(data);
                        // $('#editKendaraan').show();
                        $('#respons-modaleditpajak').html(data);
                    }
                })
            })
            // session delete pajak 
